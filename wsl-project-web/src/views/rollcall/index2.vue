<template>
  <div class="homePage aaa" @click="random">
    <canvas id="lines"></canvas>
    <h1 id="stName">class begins</h1>
  </div>
</template>

<script>
import api from '@/api/api'
import "../../stylesheets/rollcall/index.scss"
export default {
  mounted() {
    this.initCanvas()
  ...
  methods: {
    random() {
      ...
      let sss = Math.floor(Math.random() * len);
      document.getElementById('stName').innerHTML = this.tableData[sss].name
    },
    // 学生列表
    getStlistList(cid) {
      let data = {
